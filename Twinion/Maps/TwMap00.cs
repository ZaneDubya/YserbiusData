...
namespace XPT.Twinion.Maps {
    class TwMap00 : TwMap {
        ...
        protected override void FnEvent07(TwPlayerServer player, MapEventType type, bool doMsgs) {
            if ((PARTY_LEVEL(player, type, doMsgs, 12)) && (PARTY_COUNT(player, type, doMsgs) == 1)) {
        ...
        protected override void FnEvent09(TwPlayerServer player, MapEventType type, bool doMsgs) {
            ...
            protected override void FnEvent12(TwPlayerServer player, MapEventType type, bool doMsgs) {
                if ((PARTY_COUNT(player, type, doMsgs) == 1) && (PARTY_LEVEL(player, type, doMsgs, 35))) {
                    if ((GET_FLAG(player, type, doMsgs, DUNGEON, ENDGAMETELE) == 1) || (GET_FLAG(player, type, doMsgs, DUNGEON, ENDGAMETELE) == 2)) {
                        ...
                        REMOVE_WALL_ITEM(player, type, doMsgs, DOOR, HERE(player, type, doMsgs));
                        BLOCK_WALL(player, type, doMsgs, HERE(player, type, doMsgs), FACING(player, type, doMsgs));
                    }
                }
                else {
