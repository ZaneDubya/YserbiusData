...
namespace XPT.Twinion.Maps {
    class TwMap31 : TwMap {
        ...
        protected override void FnEvent12(TwPlayerServer player, MapEventType type, bool doMsgs) {
            ...
            if (GET_FLAG(player, type, doMsgs, PARTY, STORMGIANT) != HERE()) {
                if (PARTY_COUNT() == 1) {
                    SHOW_TEXT(player, type, doMsgs, "An ominous force stands before you!");
                    if (HAS_ITEM(player, type, doMsgs, STATUEOFSTORMGIANT) && GET_FLAG(player, type, doMsgs, PARTY, IMHERE) != HERE() && !GET_FLAG(player, type, doMsgs, DUNGEON, ONLYONCE)) {
                        WallClear(player, type, doMsgs);
                        ...
                        SET_FLAG(player, type, doMsgs, PARTY, IMHERE, HERE());
                    }
                }
                else if (GET_FLAG(player, type, doMsgs, PARTY, IMHERE) != HERE()) {
                ...
                SET_FLAG(player, type, doMsgs, PARTY, STORMGIANT, HERE());
            }
        }
        ...
        protected override void FnEvent16(TwPlayerServer player, MapEventType type, bool doMsgs) {
            ...
            if (!GET_FLAG(player, type, doMsgs, ROOM, SPRUNGTRAP)) {
                switch (FACING()) {
                    ...
                    case WEST:
                        ROTATE(player, type, doMsgs, EAST);
                        TELEPORT(player, type, doMsgs, 12, 2, HERE(), EAST);
                        SprungTrap(player, type, doMsgs);
                        break;
                }
            }
