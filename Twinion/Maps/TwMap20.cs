...
namespace XPT.Twinion.Maps {
    class TwMap20 : TwMap {
        ...
        protected override void FnEvent02(TwPlayerServer player, MapEventType type, bool doMsgs) {
            if (RACE(player, type, doMsgs) == GNOME || RACE(player, type, doMsgs) == TROLL) {
                TELEPORT(player, type, doMsgs, 8, 2, 251, NORTH);
            }
            else if (RACE(player, type, doMsgs) == ELF || RACE(player, type, doMsgs) == GREMLIN) {
                TELEPORT(player, type, doMsgs, 8, 2, 96, EAST);
            }
            else if (RACE(player, type, doMsgs) == DWARF || RACE(player, type, doMsgs) == ORC) {
        ...
        protected override void FnEvent04(TwPlayerServer player, MapEventType type, bool doMsgs) {
            if (RACE(player, type, doMsgs) == GNOME || RACE(player, type, doMsgs) == ELF) {
                TELEPORT(player, type, doMsgs, 8, 2, 81, EAST);
            }
            else if (RACE(player, type, doMsgs) == TROLL || RACE(player, type, doMsgs) == GREMLIN) {
                TELEPORT(player, type, doMsgs, 8, 2, 163, NORTH);
            }
            else if (RACE(player, type, doMsgs) == DWARF || RACE(player, type, doMsgs) == HALFLING) {
