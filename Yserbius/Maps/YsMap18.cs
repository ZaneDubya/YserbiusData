...
namespace XPT.Games.Yserbius.Maps {
    class YserMap18 : YsMap {
        ...
        private void FnTELEPORT_01(YsPlayerServer player, MapEventType type, bool doMsgs) {
            ...
            L0000: // BEGIN;
            L0003: TeleportParty(player, 0x05, 0x03, 0x40, 0x02, type);
            L001E: return; // RETURN;
        }
        ...
        private void FnTELEPORT_02(YsPlayerServer player, MapEventType type, bool doMsgs) {
            int ax = 0, bx = 0, cx = 0, dx = 0, si = 0, di = 0, tmp = 0;
            L0000: // BEGIN;
            L0003: TeleportParty(player, 0x04, 0x01, 0x78, 0x01, type);
            L001E: return; // RETURN;
        }
        ...
        private void FnTELEPORT_03(YsPlayerServer player, MapEventType type, bool doMsgs) {
            int ax = 0, bx = 0, cx = 0, dx = 0, si = 0, di = 0, tmp = 0;
            L0000: // BEGIN;
            L0003: TeleportParty(player, 0x05, 0x02, 0xF1, 0x03, type);
            L001E: return; // RETURN;
        }

        private void FnTELEPORT_04(YsPlayerServer player, MapEventType type, bool doMsgs) {
            int ax = 0, bx = 0, cx = 0, dx = 0, si = 0, di = 0, tmp = 0;
            L0000: // BEGIN;
            L0003: TeleportParty(player, 0x05, 0x02, 0x96, 0x00, type);
            L001D: return; // RETURN;
        }

        private void FnLAVA_05(YsPlayerServer player, MapEventType type, bool doMsgs) {
            int ax = 0, bx = 0, cx = 0, dx = 0, si = 0, di = 0, tmp = 0;
            L0000: // BEGIN;
            L0003: SetFloorItem(player, 0x02, GetCurrentTile(player));
            L0018: ShowMessage(player, doMsgs, String03FC); // You step into molten lava and your flesh burns.
            L0025: DamagePlayer(player, GetMaxHits(player));
            L0036: return; // RETURN;
        }

        private void FnSPKEYDOR_13(YsPlayerServer player, MapEventType type, bool doMsgs) {
            int ax = 0, bx = 0, cx = 0, dx = 0, si = 0, di = 0, tmp = 0;
            L0000: // BEGIN;
            ...
            L0054: ShowMessage(player, doMsgs, String042C); // The Lava Key opens the door.
            L0061: goto L00AB;
            ...
            L009E: ShowMessage(player, doMsgs, String0449); // The door is locked. It requires a special key.
            L00AB: return; // RETURN;
        }

        private void FnKEYENC_14(YsPlayerServer player, MapEventType type, bool doMsgs) {
            int ax = 0, bx = 0, cx = 0, dx = 0, si = 0, di = 0, tmp = 0;
            L0000: // BEGIN;
            L0003: ax = HasItem(player, YsIndexes.ItemLavaKey);
            ...
            L003F: goto L006D;
            L0041: AddTreasure(player, 0x1F40, 0x00, 0x00, 0x00, 0x00, YsIndexes.ItemLavaKey);
        ...
        private void FnFOUNHLTH_15(YsPlayerServer player, MapEventType type, bool doMsgs) {
            int ax = 0, bx = 0, cx = 0, dx = 0, si = 0, di = 0, tmp = 0;
            L0000: // BEGIN;
            L0003: Compare(GetFlag(player, FlagTypeMap, 0x01), 0x0001);
            L0017: if (JumpNotEqual) goto L0035;
            L0019: ShowPortrait(player, 0x0042);
            L0026: ShowMessage(player, doMsgs, String04DB); // You sip from the Fountain of Hot Ice, but nothing happens.
            L0033: goto L0071;
            L0035: HealPlayer(player, 0x07D0);
            L0042: SetFlag(player, FlagTypeMap, 0x01, 0x01);
            L0057: ShowPortrait(player, 0x0042);
            L0064: ShowMessage(player, doMsgs, String0516); // You sip from the Fountain of Hot Ice and feel your Health restored.
            L0071: return; // RETURN;
        }

        private void FnLKPKDOOR_16(YsPlayerServer player, MapEventType type, bool doMsgs) {
            int ax = 0, bx = 0, cx = 0, dx = 0, si = 0, di = 0, tmp = 0;
            L0000: // BEGIN;
            ...
            L0065: ShowMessage(player, doMsgs, String055A); // The door lock opens easily.
            L0072: goto L009E;
            L0074: SetWallPassable(player, GetCurrentTile(player), GetFacing(player), 0x00);
            L0091: ShowMessage(player, doMsgs, String0576); // The door is locked.
            L009E: return; // RETURN;
        }
        ...
        private void FnGOLDAENC_17(YsPlayerServer player, MapEventType type, bool doMsgs) {
            int ax = 0, bx = 0, cx = 0, dx = 0, si = 0, di = 0, tmp = 0;
            L0000: // BEGIN;
            L0003: Compare(GetFlag(player, FlagTypeDungeon, YsIndexes.FlagLavaCellarGold), 0x0001);
            L0017: if (JumpNotEqual) goto L0047;
            L0019: ShowMessage(player, doMsgs, String058A); // Angry Sabertooth Lions roar at you.
            L0026: AddTreasure(player, 0x07D0, 0x00, 0x00, 0x00, 0x00, 0xCF);
            L0045: goto L008A;
            L0047: AddTreasure(player, 0x2710, 0x00, 0x00, 0xB9, 0xD0, 0xD0);
            L0068: SetFlag(player, FlagTypeDungeon, YsIndexes.FlagLavaCellarGold, 0x01);
            L007D: ShowMessage(player, doMsgs, String05AE); // The lion lair you enter is littered with gold.
        ...
        private void FnGOLDBENC_18(YsPlayerServer player, MapEventType type, bool doMsgs) {
            int ax = 0, bx = 0, cx = 0, dx = 0, si = 0, di = 0, tmp = 0;
            L0000: // BEGIN;
            L0003: Compare(GetFlag(player, FlagTypeDungeon, YsIndexes.FlagLavaCellarGold1), 0x0001);
            L0017: if (JumpNotEqual) goto L0047;
            L0019: ShowMessage(player, doMsgs, String05DD); // You step into a Wyvern den.
            L0026: AddTreasure(player, 0x1B58, 0x00, 0x00, 0x00, 0x00, 0xD0);
            L0045: goto L0089;
            L0047: AddTreasure(player, 0x2EE0, 0x00, 0x00, 0x00, 0xB9, 0xB4);
            L0067: SetFlag(player, FlagTypeDungeon, YsIndexes.FlagLavaCellarGold1, 0x01);
            L007C: ShowMessage(player, doMsgs, String05F9); // Gold coins are stuck to a Wyvern's scales.
        ...
        private void FnITEMAENC_19(YsPlayerServer player, MapEventType type, bool doMsgs) {
            int ax = 0, bx = 0, cx = 0, dx = 0, si = 0, di = 0, tmp = 0;
            L0000: // BEGIN;
            L0003: ax = HasItem(player, 0xC4);
            L0011: if (JumpEqual) goto L0041;
            L0013: ShowMessage(player, doMsgs, String0624); // You discover a Knights' sanctuary.
            L0020: AddTreasure(player, 0x0BB8, 0x00, 0x00, 0x00, 0x00, 0xB7);
            L003F: goto L006E;
            L0041: AddTreasure(player, 0x2328, 0x00, 0x00, 0x00, 0xD0, 0xC4);
            L0061: ShowMessage(player, doMsgs, String0647); // A Knight drops a lockpick on the floor.
        ...
        private void FnITEMBENC_1A(YsPlayerServer player, MapEventType type, bool doMsgs) {
            int ax = 0, bx = 0, cx = 0, dx = 0, si = 0, di = 0, tmp = 0;
            L0000: // BEGIN;
            L0003: ax = HasItem(player, 0xC9);
            L0011: if (JumpEqual) goto L0042;
            L0013: ShowMessage(player, doMsgs, String066F); // This room is a Cleric stronghold.
            L0020: AddTreasure(player, 0x07D0, 0x00, 0x00, 0x00, 0xCB, 0xB2);
            L0040: goto L006F;
            L0042: AddTreasure(player, 0x2710, 0x00, 0x00, 0x00, 0xCD, 0xC9);
            L0062: ShowMessage(player, doMsgs, String0691); // A Cleric shows off a wand to his associates.
        ...
        private void FnITEMCENC_1B(YsPlayerServer player, MapEventType type, bool doMsgs) {
            int ax = 0, bx = 0, cx = 0, dx = 0, si = 0, di = 0, tmp = 0;
            L0000: // BEGIN;
            L0003: ax = HasItem(player, 0xBE);
            L0011: if (JumpEqual) goto L0041;
            L0013: ShowMessage(player, doMsgs, String06BE); // You step into a nest of Pincer Cobras.
            L0020: AddTreasure(player, 0x1388, 0x00, 0x00, 0x00, 0x00, 0xB7);
            L003F: goto L006E;
            L0041: AddTreasure(player, 0x2710, 0x00, 0x00, 0x00, 0xD0, 0xBE);
            L0061: ShowMessage(player, doMsgs, String06E5); // In a snake nest is a shining prism.
            L006E: Compare(PartyCount(player), 0x0001);
        ...
        private void FnARMRAENC_1C(YsPlayerServer player, MapEventType type, bool doMsgs) {
            int ax = 0, bx = 0, cx = 0, dx = 0, si = 0, di = 0, tmp = 0;
            L0000: // BEGIN;
            L0003: ax = HasItem(player, 0x82);
            L0011: if (JumpEqual) goto L0042;
            L0013: ShowMessage(player, doMsgs, String0709); // You interrupt a Wizards' coven.
            L0020: AddTreasure(player, 0x03E8, 0x00, 0x00, 0x00, 0xAA, 0xB9);
            L0040: goto L006F;
            L0042: AddTreasure(player, 0x1770, 0x00, 0x00, 0x00, 0xB9, 0x82);
            L0062: ShowMessage(player, doMsgs, String0729); // The chief Wizard of the coven wears a round breastplate.
            L006F: Compare(PartyCount(player), 0x0001);
        ...
        private void FnMAGICENC_1D(YsPlayerServer player, MapEventType type, bool doMsgs) {
            int ax = 0, bx = 0, cx = 0, dx = 0, si = 0, di = 0, tmp = 0;
            L0000: // BEGIN;
            L0003: ax = HasItem(player, 0x9E);
            L0011: if (JumpEqual) goto L0042;
            L0013: ShowMessage(player, doMsgs, String0762); // Rogue Rangers use this place as a safehold.
            L0020: AddTreasure(player, 0x07D0, 0x00, 0x00, 0x00, 0xB6, 0xB8);
            L0040: goto L006F;
            L0042: AddTreasure(player, 0x1770, 0x00, 0x00, 0x00, 0xD0, 0x9E);
            L0062: ShowMessage(player, doMsgs, String078E); // A sly Ranger hides something inside his jacket.
            L006F: Compare(PartyCount(player), 0x0001);
            L007A: if (JumpNotEqual) goto L00A3;
        ...
        private void FnWEAPAENC_1E(YsPlayerServer player, MapEventType type, bool doMsgs) {
            int ax = 0, bx = 0, cx = 0, dx = 0, si = 0, di = 0, tmp = 0;
            L0000: // BEGIN;
            L0003: ax = HasItem(player, 0x29);
            L0011: if (JumpEqual) goto L0041;
            L0013: ShowMessage(player, doMsgs, String07BE); // Rogue Barbarians have a wrestling center here.
            L0020: AddTreasure(player, 0x1B58, 0x00, 0x00, 0x00, 0x00, 0xAD);
            L003F: goto L006E;
            L0041: AddTreasure(player, 0x2328, 0x00, 0x00, 0x00, 0xB7, 0x29);
            L0061: ShowMessage(player, doMsgs, String07ED); // A Barbarian throws a heavy mace aside to swing at you.
            L006E: Compare(PartyCount(player), 0x0001);
            L0079: if (JumpNotEqual) goto L00A2;
        ...
        private void FnSTRMNSTR_1F(YsPlayerServer player, MapEventType type, bool doMsgs) {
            int ax = 0, bx = 0, cx = 0, dx = 0, si = 0, di = 0, tmp = 0;
            L0000: // BEGIN;
        ...
        private void FnTELMESSA_25(YsPlayerServer player, MapEventType type, bool doMsgs) {
            int ax = 0, bx = 0, cx = 0, dx = 0, si = 0, di = 0, tmp = 0;
            L0000: // BEGIN;
            L0003: ShowMessage(player, doMsgs, String0824); // There is a teleport in the north wall.
            L0010: return; // RETURN;
        }

        private void FnTELMESSB_26(YsPlayerServer player, MapEventType type, bool doMsgs) {
            int ax = 0, bx = 0, cx = 0, dx = 0, si = 0, di = 0, tmp = 0;
            L0000: // BEGIN;
            L0003: ShowMessage(player, doMsgs, String084B); // There is a teleport in the south wall.
            L0010: return; // RETURN;
        }

        private void FnTELMESSC_27(YsPlayerServer player, MapEventType type, bool doMsgs) {
            int ax = 0, bx = 0, cx = 0, dx = 0, si = 0, di = 0, tmp = 0;
            L0000: // BEGIN;
            L0003: ShowMessage(player, doMsgs, String0872); // There is a teleport in the east wall.
            L0010: return; // RETURN;
        }

        private void FnTELMESSD_28(YsPlayerServer player, MapEventType type, bool doMsgs) {
            int ax = 0, bx = 0, cx = 0, dx = 0, si = 0, di = 0, tmp = 0;
            L0000: // BEGIN;
            L0003: ShowMessage(player, doMsgs, String0898); // There is a teleport in the south wall.
            L0010: return; // RETURN;
        }

        private void FnLAVAMESS_29(YsPlayerServer player, MapEventType type, bool doMsgs) {
            int ax = 0, bx = 0, cx = 0, dx = 0, si = 0, di = 0, tmp = 0;
            L0000: // BEGIN;
            L0003: ShowMessage(player, doMsgs, String08BF); // The heat of the adjacent lava weakens you.
            L0010: DamagePlayer(player, 0x0005);
            L001D: return; // RETURN;
        }

        private void FnNPCCHATA_2A(YsPlayerServer player, MapEventType type, bool doMsgs) {
            int ax = 0, bx = 0, cx = 0, dx = 0, si = 0, di = 0, tmp = 0;
            L0000: // BEGIN;
            L0003: ShowMessage(player, doMsgs, String08EA); // You encounter a Troll Knight.
            L0010: ShowPortrait(player, 0x001B);
            L001D: Compare(GetRandom(0x000F), 0x000C);
            L002D: if (JumpAbove) goto L003E;
            L002F: ShowMessage(player, doMsgs, String0908); // The only way to get out of this basement area is to go through the lava field. Touching the lava is deadly, as you might expect. However, even if you pass safely through the lava field, the great heat of the lava will sap your strength.
            L003C: goto L004B;
            L003E: ShowMessage(player, doMsgs, String09F5); // The Troll Knight scowls angrily, causing you to leave her alone.
            L004B: return; // RETURN;
        }

        private void FnNPCCHATB_2B(YsPlayerServer player, MapEventType type, bool doMsgs) {
            int ax = 0, bx = 0, cx = 0, dx = 0, si = 0, di = 0, tmp = 0;
            L0000: // BEGIN;
            L0003: ShowMessage(player, doMsgs, String0A36); // You encounter a Dwarf Thief.
            L0010: ShowPortrait(player, 0x0023);
            L001D: Compare(GetRandom(0x000F), 0x0009);
            L002D: if (JumpAbove) goto L003E;
            L002F: ShowMessage(player, doMsgs, String0A53); // I know there is a short cut through the lava field. If you can find the correct key, you will be able to use the short cut. The key also will lead you to riches.
            L003C: goto L004B;
            L003E: ShowMessage(player, doMsgs, String0AF5); // The Dwarf Thief runs away.
            L004B: return; // RETURN;
        }

        private void FnNPCCHATC_2C(YsPlayerServer player, MapEventType type, bool doMsgs) {
            int ax = 0, bx = 0, cx = 0, dx = 0, si = 0, di = 0, tmp = 0;
            L0000: // BEGIN;
            L0003: ShowMessage(player, doMsgs, String0B10); // You encounter a Halfling Ranger.
            L0010: ShowPortrait(player, 0x0021);
            L001D: Compare(GetRandom(0x000F), 0x0007);
            L002D: if (JumpAbove) goto L003E;
            L002F: ShowMessage(player, doMsgs, String0B31); // The dwarves who built Cleowyn's Palace transported precious metals and building blocks up to the top level by means of a teleport. I believe the teleport is somewhere in this area.
            L003C: goto L004B;
            L003E: ShowMessage(player, doMsgs, String0BE6); // The Halfling Ranger faints from heat stroke.
            L004B: return; // RETURN;
        }

        private void FnNPCCHATD_2D(YsPlayerServer player, MapEventType type, bool doMsgs) {
            int ax = 0, bx = 0, cx = 0, dx = 0, si = 0, di = 0, tmp = 0;
            L0000: // BEGIN;
            L0003: ShowMessage(player, doMsgs, String0C13); // You encounter a Gremlin Cleric.
            L0010: ShowPortrait(player, 0x002A);
            L001D: Compare(GetRandom(0x000F), 0x000C);
            L002D: if (JumpAbove) goto L003E;
            L002F: ShowMessage(player, doMsgs, String0C33); // Deep in the dungeon are the Pillow Labyrinths. They were formed from pillow lava that spilled into the sea. The labyrinth is formed from the hollow interior of the lava.
            L003C: goto L004B;
            L003E: ShowMessage(player, doMsgs, String0CDD); // The Gremlin Cleric is deep in meditation and does not answer you.
            L004B: return; // RETURN;
        }
