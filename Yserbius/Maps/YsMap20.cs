    ...
    class YserMap20 : AMapScripted {
        ...
        private void DoDefeatWind(ServerPlayer player, int windIndex, string windName) {
            ...
            if (countAlreadyDefeated == 0) {
                ShowMessage(player, true, $"The {windName} is the first to be defeated.");
            }
            else {
                ShowMessage(player, true, $"The {windName} has been quieted.");
            }
            ...
            if (countNowDefeated == 4) {
                ShowMessage(player, true, "A stillness fills the labryinth.");
            }
        }
        ...
        private void FnEXPRESS_11(ServerPlayer player, MapEventType type, bool doMsgs) {
            ...
            ShowMessage(player, doMsgs, String095D); // This wall is the teleport to CASTLEGATE.
        ...
        private void FnWSTRES_13(ServerPlayer player, MapEventType type, bool doMsgs) {
            ...
            if (GetFlag(player, FlagTypeDungeon, FlagModAttr_WynFountain) == 0) {
                ...
                ShowMessage(player, doMsgs, String0A6D); // It seems to instantly evaporate into a large steam cloud.
                ShowMessage(player, doMsgs, String0AA7); // You feel you may be quicker in a fight.
                return;
            }
        ...
        private void FnSDOORWKT_19(ServerPlayer player, MapEventType type, bool doMsgs) {
            ...
            if (AreFourWindsDefeated(player)) {
                ShowMessage(player, doMsgs, "'You have defeated the four winds! We cannot hold you back. Enter the WIND KNIGHTS' TOMB if you dare.");
                SetWallPassable(player, GetCurrentTile(player), 0x03, 0x01);
                SetWallItem(player, 0x01, GetCurrentTile(player), 0x03);
            }
            else {
                ShowMessage(player, doMsgs, "'Only Forces more powerful than the four winds can open this door!'");
            }
        }
        ...
        private void FnKEYDOORW_1B(ServerPlayer player, MapEventType type, bool doMsgs) {
            if (HasUsedItem(player, type, ref doMsgs, ItemWindKnightsKey, ItemWindKnightsKey) == 1) {
                ...
                ShowMessage(player, doMsgs, String0DB7); // The soft breezes seem to whisper - 'Beware, those who seek the treasure of WIND KNIGHTS' TOMB.'
                return;
            }
        ...
        private void FnSIRDOOR_28(ServerPlayer player, MapEventType type, bool doMsgs) {
            ...
            L004C: ShowMessage(player, doMsgs, String11FA); // Sirocco's powers no longer hold this door.  It is open.
            L0059: goto L0075;
            L005B: ShowMessage(player, doMsgs, String1232); // Sirocco's Door does not open.  In the distance, a wizard laughs aloud.
            L0068: ShowMessage(player, doMsgs, String1279); // 'It is I, Sirocco, who bars your way!'
            L0075: return; // RETURN;
        }
        ...
        private void FnWSBIGDR_2A(ServerPlayer player, MapEventType type, bool doMsgs) {
            ...
            L0014: ShowMessage(player, doMsgs, String1303); // You force the massive door open by sheer muscle power.
            L0021: SetWallPassable(player, GetCurrentTile(player), GetFacing(player), 0x01);
            L003F: goto L006B;
            L0041: ShowMessage(player, doMsgs, String133A); // The door is stuck. You are not strong enough to force it open.
        ...
        private void FnSDOORWS_34(ServerPlayer player, MapEventType type, bool doMsgs) {
            ...
            L0057: ShowMessage(player, doMsgs, String1521); // You found a secret door!
            L0064: return; // RETURN;
        }
        ...
        private void FnARKARCH_35(ServerPlayer player, MapEventType type, bool doMsgs) {
            ...
            L0010: ShowMessage(player, doMsgs, String155E); // 'Labyrinth and lower levels first explored by Arkarch of the Snow Elves.'
            L001D: return; // RETURN;
        }
        ...
        private void FnFNTNHEAL_37(ServerPlayer player, MapEventType type, bool doMsgs) {
            ...
            L0021: ShowMessage(player, doMsgs, String15DF); // Your many wounds and lingering diseases are removed by the sweet waters of WindyBless Fountain.
            L002E: return; // RETURN;
        }
        ...
        private void FnFNTNMANA_38(ServerPlayer player, MapEventType type, bool doMsgs) {
            ...
            L001D: ShowMessage(player, doMsgs, String163F); // The mystic waters of the NightWillow Fountain restore your Mana level.
            L002A: return; // RETURN;
        }
        ...
        private void FnSDOORLTA_3A(ServerPlayer player, MapEventType type, bool doMsgs) {
            ...
            L006C: ShowMessage(player, doMsgs, String16B0); // You found a secret door!
            L0079: return; // RETURN;
        }
        ...
        private void FnSDOORLTB_3B(ServerPlayer player, MapEventType type, bool doMsgs) {
            ...
            L004C: if (JumpEqual) goto L0079;
            L004E: SetWallPassable(player, GetCurrentTile(player), GetFacing(player), 0x01);
            L006C: ShowMessage(player, doMsgs, String16C9); // You discover a secret door!
            L0079: return; // RETURN;
        }
        ...
        private void FnSDOORLTC_3C(ServerPlayer player, MapEventType type, bool doMsgs) {
            ...
            L004C: if (JumpEqual) goto L0079;
            L004E: SetWallPassable(player, GetCurrentTile(player), GetFacing(player), 0x01);
            L006C: ShowMessage(player, doMsgs, String16E5); // A secret door suddenly appears!
            L0079: return; // RETURN;
        }
        ...
        private void FnBDOORLTD_3D(ServerPlayer player, MapEventType type, bool doMsgs) {
            ...
            L0014: ShowMessage(player, doMsgs, String1705); // You bash the massive door open.
            L0021: SetWallPassable(player, GetCurrentTile(player), GetFacing(player), 0x01);
            L003F: return; // RETURN;
        }
        ...
        private void FnLTSKILL_45(ServerPlayer player, MapEventType type, bool doMsgs) {
            ...
            L005E: ShowMessage(player, doMsgs, String182F); // You gain the skill of Detection!
            L006B: goto L010B;
            L006E: RefreshCompareFlags(GetSkillLevel(player, 0x0E));
            L007C: if (JumpNotEqual) goto L009E;
            L007E: SetSkillLevel(player, 0x0E, 0x01);
            L008F: ShowMessage(player, doMsgs, String1850); // You gain the skill of Lockpicking!
            L009C: goto L010B;
            L009E: RefreshCompareFlags(GetSkillLevel(player, 0x0C));
            L00AC: if (JumpNotEqual) goto L00CE;
            L00AE: SetSkillLevel(player, 0x0C, 0x02);
            L00BF: ShowMessage(player, doMsgs, String1873); // You gain the Bard skill!
            L00CC: goto L010B;
            L00CE: RefreshCompareFlags(GetSkillLevel(player, 0x0F));
            L00DC: if (JumpNotEqual) goto L00FE;
            L00DE: SetSkillLevel(player, 0x0F, 0x01);
            L00EF: ShowMessage(player, doMsgs, String188C); // You gain the Pickpocketing skill!
            L00FC: goto L010B;
            L00FE: ShowMessage(player, doMsgs, String18AE); // Nothing happens.
            L010B: goto L011A;
            L010D: ShowMessage(player, doMsgs, String18BF); // The shrines does nothing.
            L011A: return; // RETURN;
        }
        ...
        private void FnTXTAWIND_49(ServerPlayer player, MapEventType type, bool doMsgs) {
            ...
            ShowMessage(player, doMsgs, String19B7); // I laughed when you troubled King Cleowyn's ghost!
            ShowMessage(player, doMsgs, String19E9); // But now you enter my tunnels!  Go back whence you came!
            ShowMessage(player, doMsgs, String1A21); // The Winds will keep these places clean.'
        }
