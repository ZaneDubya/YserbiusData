...
namespace XPT.Legacy.Maps {
    class YserMap02 : AMapScripted {
        ...
        private void FnKEYDOOR_03(ServerPlayer player, MapEventType type, bool doMsgs) {
            ...
            L0061: goto L008D;
            L0063: ShowMessage(player, doMsgs, String0421); // The door is locked.  Something special is needed to open this door.
        ...
        private void FnKEYMESSG_04(ServerPlayer player, MapEventType type, bool doMsgs) {
            ...
            L0010: ShowRunes(player, doMsgs, String048E); // A certain Key unlocks my series of doors.
            L001D: return; // RETURN;
        }
        ...
        private void FnLKPKDOOR_05(ServerPlayer player, MapEventType type, bool doMsgs) {
            ...
            L0043: SetWallPassable(player, GetCurrentTile(player), GetFacing(player), 0x01);
            L0061: goto L008D;
            L0063: ShowMessage(player, doMsgs, String04E2); // The door is locked.
            L0070: SetWallPassable(player, GetCurrentTile(player), GetFacing(player), 0x00);
            L008D: return; // RETURN;
        }
        ...
        private void FnLKPKMESG_06(ServerPlayer player, MapEventType type, bool doMsgs) {
            ...
            L0010: ShowRunes(player, doMsgs, String0519); // A certain Lockpick opens my series of doors.
            L001D: return; // RETURN;
        }
        ...
        private void FnSTRDOOR_07(ServerPlayer player, MapEventType type, bool doMsgs) {
            ...
            L0043: SetWallPassable(player, GetCurrentTile(player), GetFacing(player), 0x01);
            L0061: goto L008D;
            L0063: ShowMessage(player, doMsgs, String0565); // The door is currently impassable.
            L0070: SetWallPassable(player, GetCurrentTile(player), GetFacing(player), 0x00);
            L008D: return; // RETURN;
        }
        ...
        private void FnSTRMESSG_08(ServerPlayer player, MapEventType type, bool doMsgs) {
            ...
            L0010: ShowRunes(player, doMsgs, String05A1); // A hero of strength may open my series of doors.
            L001D: return; // RETURN;
        }
        ...
        private void FnLOWMNSTR_0D(ServerPlayer player, MapEventType type, bool doMsgs) {
            ...
            L0041: ShowMessage(player, doMsgs, String06BF); // If you are skilled enough, you may find teleports that speed your way to various levels of this dungeon.
            L004E: goto L008A;
            L0050: ShowMessage(player, doMsgs, String0728); // The Rogues snarl at you.
            L005D: ShowMessage(player, doMsgs, String0741); // They attack!
        ...
        private void FnREGDOOR_16(ServerPlayer player, MapEventType type, bool doMsgs) {
            ...
            L0043: SetWallPassable(player, GetCurrentTile(player), GetFacing(player), 0x01);
            L0061: goto L008D;
            L0063: ShowMessage(player, doMsgs, String082F); // The door is currently impassable.
            L0070: SetWallPassable(player, GetCurrentTile(player), GetFacing(player), 0x00);
            L008D: return; // RETURN;
        }
        ...
        private void FnNPCCHATA_17(ServerPlayer player, MapEventType type, bool doMsgs) {
            ...
            L002F: ShowMessage(player, doMsgs, String086E); // You must solve the mystery of the linked Vestibule doors to continue farther down into the dungeon.
            L003C: goto L004B;
            L003E: ShowMessage(player, doMsgs, String08D2); // The Elf Ranger decides you are not worthy of his time and leaves.
            L004B: return; // RETURN;
        }
        ...
        private void FnNPCCHATB_18(ServerPlayer player, MapEventType type, bool doMsgs) {
            ...
            L002D: if (JumpAbove) goto L003E;
            L002F: ShowMessage(player, doMsgs, String0932); // Centuries ago, Twinion was a thriving port town, ruled by the Galabryan dynasty. The kings welcomed all races and Guilds to the island. When the volcano Yserbius erupted, it almost buried Twinion under lava. If only we could find some way off this island!
            L003C: goto L004B;
            L003E: ShowMessage(player, doMsgs, String0A32); // The Human Wizard falls into a deep trance and refuses to talk.
            L004B: return; // RETURN;
        }
        ...
        private void FnNPCCHATC_19(ServerPlayer player, MapEventType type, bool doMsgs) {
            ...
            L002D: if (JumpAbove) goto L003E;
            L002F: ShowMessage(player, doMsgs, String0A90); // King Cleowyn created a clever maze of squares to discourage thieves and invaders. Mind you, no invaders ever thought Twinion rich enough to be worth their effort.
            L003C: goto L004B;
            L003E: ShowMessage(player, doMsgs, String0B33); // The Gremlin Thief slips into a shadow and disappears.
            L004B: return; // RETURN;
        }
        ...
        private void FnNPCCHATD_1A(ServerPlayer player, MapEventType type, bool doMsgs) {
            ...
            L002D: if (JumpAbove) goto L003E;
            L002F: ShowMessage(player, doMsgs, String0B86); // The richness of fine armor and weapons in this dungeon baffles me. I've heard rumors that skilled dwarfs once crafted such armory, but the rogue dwarfs you encounter nowadays have no talent for such expert metallurgy.
            L003C: goto L004B;
            L003E: ShowMessage(player, doMsgs, String0C60); // The Orc Knight smiles politely and shoos you away.
            L004B: return; // RETURN;
        }
        ...
        private void FnNPCCHATE_1B(ServerPlayer player, MapEventType type, bool doMsgs) {
            ...
            L001D: Compare(GetRandom(0x000F), 0x0008);
            L002D: if (JumpAbove) goto L003E;
            L002F: ShowMessage(player, doMsgs, String0CB0); // I have visited King Cleowyn's throne room, and what a disappointment it was. Not much treasure there and too many dead things. Perhaps I missed something.
            L003C: goto L004B;
            L003E: ShowMessage(player, doMsgs, String0D4B); // The Elf Cleric is too engrossed in his prayers to talk with you.
            L004B: return; // RETURN;
        }
        ...
        private void FnKEYDRA_1C(ServerPlayer player, MapEventType type, bool doMsgs) {
            ...
            L0070: goto L009C;
            L0072: ShowMessage(player, doMsgs, String0DAC); // The door is locked.
        ...
        private void FnLKPKDRA_1D(ServerPlayer player, MapEventType type, bool doMsgs) {
            ...
            L0052: SetWallPassable(player, GetCurrentTile(player), GetFacing(player), 0x01);
            L0070: goto L009C;
            L0072: ShowMessage(player, doMsgs, String0DEA); // The door is locked.  It looks like something special is needed to open this door.
            L007F: SetWallPassable(player, GetCurrentTile(player), GetFacing(player), 0x00);
            L009C: return; // RETURN;
        }
        ...
        private void FnSTRNGDRA_1E(ServerPlayer player, MapEventType type, bool doMsgs) {
            ...
            L0071: goto L009D;
            L0073: ShowMessage(player, doMsgs, String0E5F); // The door is currently impassable.
        ...
        private void FnREGDORA_1F(ServerPlayer player, MapEventType type, bool doMsgs) {
            ...
            L0089: goto L00B5;
            L008B: ShowMessage(player, doMsgs, String0EC4); // The door is currently impassable.
